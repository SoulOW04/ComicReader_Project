...
public class Admin_Link_Update extends AppCompatActivity {

    IComicAPI myAPI;
    INodeJS nodeAPI;
    Spinner spinnerLinkId,SpinnerLinkId_update;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_link_update);

        edit_Link = findViewById(R.id.edt_link);
        //id from
        //spinnerChapterId =(Spinner) findViewById(R.id.link_id_chapter);
        spinnerLinkId =(Spinner) findViewById(R.id.link_id);
        SpinnerLinkId_update = (Spinner)findViewById(R.id.link_id_update);

        List<Chapter> chapters = new ArrayList<>();
        List<Link> links = new ArrayList<>();

        ArrayAdapter<Chapter> adapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item,chapters);
        ArrayAdapter<Link> adapterLink = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item,links);
        ...
        SpinnerLinkId_update.setAdapter(mAdapter);



        SpinnerLinkId_update.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                //Toast.makeText(Admin_Link_Update.this, clickedLink + " selected", Toast.LENGTH_SHORT).show();
            }
        ...
        //Call<List<Chapter>> callChapter = myAPI.getChapterList2();
        //Call<List<Link>> callLink = myAPI.getLinkList2();
        ...
        callLink.enqueue(new Callback<List<Link>>() {
            @Override
            public void onResponse(Call<List<Link>> callLink, Response<List<Link>> response) {
                if(response.isSuccessful()){
                    ...
                    spinnerLinkId.setAdapter(adapterLink);
                }
            }
            @Override
            public void onFailure(Call<List<Link>> call, Throwable t) {
                Toast.makeText(Admin_Link_Update.this,""+call,Toast.LENGTH_SHORT).show();
            }
        });
...
//        deleteManga(1);
        ...
        btn_Update.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Link selectedLink = (Link)spinnerLinkId.getSelectedItem();
                int ID = selectedLink.getID();
                //chapter primary id
                int chapterId =  Common.selected_chapter.getID();
                String Link = edit_Link.getText().toString();
                updateLink(ID,Link,chapterId);
            }
        });
    }
    ...
    private void updateLink(int id,String link, int chapterid) {
        if(id == 0 || link.equals("") || chapterid == 0){
            Toast.makeText(Admin_Link_Update.this,"Please input all form",Toast.LENGTH_SHORT).show();;
        }else{
        // View register_layout = LayoutInflater.from(this).inflate(R.layout.activity_admin_manga_add, null);
        compositeDisposable
                .add(nodeAPI.updateLink(id,link,chapterid)
                        .subscribeOn(Schedulers.io())   
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(new Consumer<String>() {
                            @Override
                            public void accept(String s) throws Exception {
                                Toast.makeText(Admin_Link_Update.this,""+s,Toast.LENGTH_SHORT).show();
                                startActivity(new Intent(Admin_Link_Update.this, ViewDetail.class));
                            }
